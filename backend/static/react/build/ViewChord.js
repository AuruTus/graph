var ChordFilter = React.createClass({
    ...
    render: function () {
        return React.createElement(
            "form",
            { onSubmit: this.handleSubmit, ref: "ChordFilterForm" },
            React.createElement(
                "div",
                null,
                React.createElement(NodeData, null)
            ),
...
function ChordLayout(containerID, id) {
    ...
    var fill = d3.scale.ordinal().domain(d3.range(8)).range(["#000000", "#FFDD89", "#957244", "#F26223", "#F26223", "#026223", "#F06223", "#F26220"]);

    // Определяем svg-сцену
    var svg = d3.select(containerID).append("svg").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 4 + ")");
    //.attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");

    d3.json(url, function (error, graph) {
        this.graph = graph;

        var chord = d3.layout.chord().padding(.05).sortSubgroups(d3.descending).matrix(graph.matrix);
        ...
        var ticks = svg.append("g").selectAll("g").data(chord.groups).enter().append("g").selectAll("g").data(groupTicks.bind(this)).enter().append("g").attr("transform", function (d) {
